...
recommendRouter.post("/post/create", login_required, async (req, res, next) => {
  try {
    if (
    ...
    const post = await recommendService.addPost({ newPost });
...
postRouter.get("/postlist", login_required, async (req, res, next) => {
  try {
    ...
    const postlist = await recommendService.getPosts({ page, perPage });
...
postRouter.get("/postlist/:userId", login_required, async (req, res, next) => {
  try {
    ...
    const post = await recommendService.getPostsByUserId({ getPosts });
...
postRouter.get("/posts/following", login_required, async (req, res, next) => {
  try {
    ...
    const followingPosts = await recommendService.getFollwingPostsByUserId({
      getPosts,
    });
...
postRouter.get("/posts/popular", login_required, async (req, res, next) => {
  try {
    ...
    const postlist = await recommendService.getPopularPosts({ type, count });
...
postRouter.get("/post/:postId", login_required, async (req, res, next) => {
  try {
    ...
    const post = await recommendService.getPostById({ id: postId });
...
postRouter.put("/post/:postId", login_required, async (req, res, next) => {
  try {
    ...
    const updatedPost = await recommendService.updatePost({
      id,
      toUpdate,
    });
...
postRouter.delete("/post/:postId", login_required, async (req, res, next) => {
  try {
    ...
    const result = await recommendService.deletePost({ id });
...
postRouter.put("/posts/like", login_required, async (req, res, next) => {
  try {
    ...
    const updatedPost = await recommendService.addLike({ id, toUpdate });
...
postRouter.put("/posts/dislike", login_required, async (req, res, next) => {
  try {
    ...
    const updatedPost = await recommendService.deleteLike({ id, toUpdate });
